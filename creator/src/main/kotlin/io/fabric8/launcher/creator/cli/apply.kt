...
import io.fabric8.launcher.creator.core.deploy.applyDeployment
...
class Apply : CliktCommand(help = "Adds capabilities to projects") {
    ...
    override fun run() {
        ...
        val deployment = DeploymentDescriptor.build {
            applications = ApplicationDescriptor.list({
                application = name
                parts = PartDescriptor.list({
