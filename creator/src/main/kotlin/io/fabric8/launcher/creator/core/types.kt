...
fun propsOf(): Properties = mutableMapOf<String, Any?>().withDefault { _ -> null }

fun propsOf(vararg pairs: Pair<String, Any?>): Properties = mutableMapOf(*pairs).withDefault { _ -> null }

fun propsOfNN(vararg pairs: Pair<String, Any?>) : Properties {
    val m = mutableMapOf<String, Any?>().withDefault { _ -> null }
...
interface BaseProperties : Properties {
    ...
    companion object {
        inline fun <reified T : BaseProperties.Data> build(
    ...
    open class Data(map: Properties = propsOf()) : BaseProperties, Properties by map {
        protected val _map: Properties = map.withDefault { _ -> null }
...
inline fun <reified T> ensureObject(name: String, obj: Any?, klazz: (Properties) -> T): T {
    if (obj is T) {
        ...
        throw RuntimeException("No value found for required property '${name}'")
    } else {
        throw RuntimeException("Unexpected type for property '${name}'")
    }
}
...
inline fun <reified T> ensureList(name: String, list: Any?, klazz: (Properties) -> T): List<T> {
    if (list is List<*>) {
        if (list.isEmpty() || list.all { it is T }) {
            ...
            return list.map { ensureObject(name, it, klazz) }.toMutableList()
        }
    } else {
...
fun envOf(vararg pairs: Pair<String, Any>) = mutableMapOf<String, Any>(*pairs)
